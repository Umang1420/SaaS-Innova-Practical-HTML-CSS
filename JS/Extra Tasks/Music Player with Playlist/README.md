# Music Player with Playlist

A simple music player that plays songs from a built-in list and lets users build a custom playlist.

## How it works
- Select a song from the available list.
- The player shows the song image and audio controls.
- You can create a playlist by entering song names.
- You can view your playlist and add songs to it.

## How to run
- Open index.html in a browser or use Live Server.
- Make sure the audio files in the songs folder are present.
